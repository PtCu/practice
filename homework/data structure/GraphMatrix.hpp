...
public:
	...
	{ //插入顶点，返回编号
		for (int j = 0; j < n; j++)	//每一行的末尾插入
			E[j].insert(NULL);
		n++;												  //各顶点预留一条潜在的关联边
		E.insert(Vector<Edge<Te> *>(n, n, (Edge<Te> *)NULL)); //创建新顶点对应的边向量。容量为n，规模为n，所有元素初始化为Null
	...
	{								//删除第i个顶点及其关联边（0 <= i < n）
		...
		E.remove(i);
		n--; //删除第i行
		Tv vBak = vertex(i);	//在点集中查询第i个顶点的信息
		V.remove(i);				//删除顶点i
		for (int j = 0; j < n; j++) //所有入边
			if (Edge<Te> *x = E[j].remove(i))	//向量中的删除
			{
				delete x;
				V[j].outDegree--;
				e--;
